...
class eCMS_Controller_Action_Helper_GenerateMenu extends Zend_Controller_Action_Helper_Abstract
{
    ...
    public function getMenu ($user = false)
    {
        ...
        if (! $user)
            $menu['member'][] = array(
            'uri' => '/members/auth/login', 
            'label' => 'Login', 
            'type' => 'navig');
        if ($user)
            $menu['member'][] = array(
            'uri' => '/members/submissions/view', 
            'label' => 'My Submissions', 
            'type' => 'navig');
        if ($user)
            $menu['member'][] = array(
            'uri' => '/members/password/change', 
            'label' => 'Change Password', 
            'type' => 'navig');
        if ($user)
            $menu['member'][] = array(
            'uri' => '/members/email/change', 
            'label' => 'Change Email Address', 
            'type' => 'navig');
        if (! $user)
            $menu['member'][] = array(
            'uri' => '/members/password/forgot', 
            'label' => 'Forgot Password', 
            'type' => 'navig');
        if ($user)
            $menu['member'][] = array(
            'uri' => '/members/auth/logout', 
            'label' => 'Logout', 
            'type' => 'navig');
        if (isset($user->uname) && $user->uname ==
         'admin') :
            $menu['admin'][] = array(
            'uri' => '/admin/settings', 
            'label' => 'Settings');
